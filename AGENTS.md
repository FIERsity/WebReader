...
- The first supported formats are DRM-free EPUB, PDF, TXT, and Markdown. Markdown uses a bounded local source-range-aware renderer for headings, inline emphasis, lists, quotes, code, and safe links; raw HTML and unsafe links never execute, and images never load remote resources. DRM circumvention is out of scope.
...
- `src/components/`: reading settings, recursive outline panels, the cross-format in-book search panel, and the root interface recovery boundary.
...
- `src/lib/markdownDocument.ts`: bounded source-range-aware Markdown block and inline parsing for safe local rendering.
...
- `src/lib/readerSearch.ts` and `src/lib/pdfSearch.ts`: bounded, cancellable in-memory matching, excerpts, PDF text-item joining, and source-fragment mapping.
...
- `src/lib/epubValidation.ts`: bounded EPUB ZIP/container preflight for entry paths, duplicates, encryption, expanded sizes, compression ratios, and required package metadata.
- `src/lib/coalescingWriteQueue.ts`: serial, per-key coalescing for progress and preference writes, with explicit flush points on reader exit and page lifecycle events.
...
The stack is React, TypeScript, Vite, Dexie, foliate-js, PDF.js, Lucide, Vitest, Playwright, Oxlint, and vite-plugin-pwa. EPUB, PDF, and text use separate reader adapters, capabilities, outline sources, locator types, and source-aware search implementations behind one reader search contract. Search queries, excerpts, result maps, and PDF text-item caches stay in memory for the active reader, are bounded and cancellable, and are never persisted or uploaded. EPUB cleanup is idempotent so async initialization and React unmount cannot release the same renderer twice; bounded ZIP/container preflight rejects encrypted, malformed, path-unsafe, duplicate, or excessive expanded EPUB resources before import. The root error boundary keeps a local recovery path if a reader still fails unexpectedly. Reflowable EPUB/text support local typography and background preferences; fixed-layout EPUB/PDF do not expose typography controls. TXT and Markdown paged mode lays source-aware blocks into viewport-sized horizontal CSS columns, while scroll mode keeps a continuous vertical document; Markdown block and inline nodes retain source ranges for search highlighting and locator restoration, and raw HTML or remote resources are never executed or loaded. Both modes persist source character offsets so typography and viewport reflow do not invalidate progress. Progress and preferences use serial, coalesced IndexedDB writes and flush on reader exit, page hiding, and page lifecycle exit. The active document's paged/scroll mode is changed from the reader toolbar and remembered per document; fixed-layout EPUB disables that control. PDF Canvas remains the authoritative visual source and has an official PDF.js TextLayer overlay for selection. A PDF document is parsed once per opened file and reused across paged, continuous, article, and proof views; page Canvas, TextLayer, continuous slots, and proof previews report failures locally without replacing other readable pages. Bounded local analysis preserves text-item provenance, stable source blocks, deterministic reading order, repeated-margin removal, page-quality diagnostics, and page-relative locators. PDF search independently scans the PDF.js text layer and maps the selected match back to source geometry without requiring or changing paper reflow. The PDF tool exposes a continuous single-column article view and a page-by-page proof view beside original page previews. Content-translation code, provider configuration, remote execution, development proxying, and production translation entries are absent; only the existing Dexie v3/v4 store names remain for non-destructive database compatibility and cascading cleanup when a book is deleted.
